(function () {
  function renderBody(blocks) {
    return (blocks || []).map(function (b) {
      if (b.type === 'h2') return '<h2 class="article-h2">' + b.text + '</h2>';
      if (b.type === 'quote') return '<figure class="article-quote">' +
        '<blockquote>' + b.text + '</blockquote>' +
        (b.cite ? '<figcaption>' + b.cite + '</figcaption>' : '') +
      '</figure>';
      if (b.type === 'list') return '<ul class="article-list">' +
        (b.items || []).map(function (it) { return '<li>' + it + '</li>'; }).join('') +
      '</ul>';
  ...
  function initArticle() {
    var wrap = document.getElementById('article-root');
    if (!wrap) return;

    var all     = window.NEWS_ARTICLES || [];
    var id      = new URLSearchParams(window.location.search).get('id');
    var article = id ? all.find(function (a) { return a.id === id; }) : all[0];
    ...
    var related = all.filter(function (a) { return a.id !== article.id; }).slice(0, 2);
            ...
            '<div class="news-grid">' +
              related.map(function (r) {
                return '<div class="reveal">' +
                  '<a class="news-card" href="news-article.html?id=' + encodeURIComponent(r.id) + '">' +
                    '<div class="news-meta">' +
      ...
      '<article class="article">' +
        '<div class="container article-container">' +
          ...
          '<div class="article-meta reveal">' +
            ...
            '<span class="article-date">' + (article.dateLong || article.date) + '</span>' +
            (article.readTime
              ? '<span class="article-dot"></span><span class="article-read">읽는 시간 ' + article.readTime + '</span>'
              : '') +
          '</div>' +
          ...
          '<div class="reveal"><div class="article-body">' + renderBody(article.body) + '</div></div>' +
          '<div class="article-foot reveal">' +
            '<div class="article-foot-author">' +
              '<div class="article-foot-author-tag">By</div>' +
              '<div class="article-foot-author-name">' + (article.author || 'PRIENZ 편집팀') + '</div>' +
            '</div>' +
